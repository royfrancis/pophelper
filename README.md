...
df <- tabulateRunsStructure(files=slist)
#tabulate TESS runs
...
plotRuns(files=slist[1:2], imgoutput="sep", poplab=pops$V1) 
...
plotRuns(files=tlist[1:2], imgoutput="sep", poplab=labs1) 
plotRuns(files=tlist[1:2], imgoutput="join", poplab=labs1) 
```
...
```coffee
# plot multiline
plotMultiline(files=slist[1]) 
plotMultiline(files=tlist[1]) 

# plotmultiline with custom setting
plotMultiline(files=slist[1], spl=75, lpp=10)
...
[Francois, O., and Durand, E. (2010). Spatially explicit Bayesian clustering models in population genetics. Molecular Ecology Resources, 10(5), 773-784.](http://onlinelibrary.wiley.com/doi/10.1111/j.1755-0998.2010.02868.x/abstract)
