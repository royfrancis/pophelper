...
A typical STRUCTURE run produces a large number of output run files as seen in __Fig. 1__.  

![Structure runs view](smallfigs/Fig1.jpg)  
__Fig. 1.__ *Typical view of Structure output run files.*
...
?tabulateRunsStructure
tabulateRunsStructure(files=NULL, writetable=FALSE, exportdataformat="excel", 
sorttable=TRUE, quiet=FALSE)
# To write results to working directory, set writetable=TRUE
# To change export format change exportdataformat="excel" or exportdataformat="txt"
...
evannoMethodStructure(data=NULL, writetable=FALSE, exportdataformat="excel", 
doplot=TRUE, exportplot=FALSE, na.rm=TRUE, imgtype="png", height=NA, width=NA, 
res=NA, units=NA)
# To write result to working directory, set writetable=TRUE
# To choose export data format, set exportdataformat="excel" or exportdataformat="txt"
...
em <- evannoMethodStructure(data=sr1,doplot=F)
...
__Fig. 2.__ *Plots produced from the Evanno method.*
...
From the command above, a png file and text file is exported to the working directory. The peak of deltaK in __Fig. 2.__ plot (D) is usually used to estimate the best value of K.
...
__Fig. 3.__ *Folders created from clumpp export and the contents of each folder.*
...
__Fig. 4.__ *Folder showing CLUMPP results: aligned file, merged file and misc file.*
...
__Fig. 5.__ *(A) Left: Single run plotted separately. (B) Right: Two runs joined together in one image.*

Population labels can be added to this barplot by providing a vector of labels. The length must be equal to the number of individuals. The labelling is still experimental and may not always give perfect results. In this demonstration, we will use the labels table in the `r print(pkgname)` library. In case of separate plots and joined plots, the labels are shown only once at the bottom of the plot.
...
__Fig. 6.__ *(A) Left: Single run plotted separately with pop labels. (B) Right: Two runs joined together in one image with pop labels.*
...
__Fig. 7.__ *(A) Left: Combined files (Three STRUCTURE runs for K=4). (B) Middle: Aligned files (Three STRUCTURE runs for K=4 aligned using CLUMPP). (C) Right: Merged file (Three runs for K=4 merged into one table/figure using CLUMPP)*
...
__Fig. 8.__ *Left: Default output from `plotMultiline`. Right: Modified output.*
...
__Fig. 12.__ *Interpolation plots showing some of the colour palettes available in package. Left 2 plots are `brewer.pal(8,"RdYlBu")`, 3^rd^ plot is `brewer.pal(8,"Spectral")` and 4^th^ plot is `brewer.pal(8,"BuPu")`.*  
