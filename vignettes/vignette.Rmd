...
tabulateRunsAdmixture()   # Get a tabulation of several ADMIXTURE files
summariseRunsAdmixture()  # Summarise runs by repeats for each K
runsToDfAdmixture()       # Convert ADMIXTURE run files to R dataframe
clumppExportAdmixture()   # Export data file and parameter file for use with CLUMPP
...
analyseRuns()             # A wrapper function to quickly tabulate, summarise, 
                          # perform evanno method, clumpp output and 
                          # generate barplots from STRUCTURE, TESS or ADMIXTURE run files.
```
...
__4.2 tabulateRunsStructure/tabulateRunsTess/tabulateRunsAdmixture__  

The functions `tabulateRunsStructure()`, `tabulateRunsTess()` or `tabulateRunsAdmixture` is used to select any number of run files in a folder and produce a table of runs with various parameters. The only mandatory argument required for this function is a character vector of filenames. The results can be pointed to a variable for further use. `tabulateRunsStructure()` is used for STRUCTURE run files, `tabulateRunsTess()` for TESS run files and `tabulateRunsAdmixture` for ADMIXTURE run files.

A typical STRUCTURE run produces a large number of output run files as seen below.  

![Structure runs view](Fig1.jpg)  
...
df <- tabulateRunsAdmixture(files=choose.files(multi=TRUE))
...
tr3 <- tabulateRunsTess(files=alist)
...
The `tabulateRunsTess()` and `tabulateRunsAdmixture` functions return a table with three columns namely filename, *K* and number of individuals.

By default, the `tabulateRunsStructure()`, `tabulateRunsTess()` and `tabulateRunsAdmixture` functions print the number of selected files as `Number of files selected: 10`. This can be turned off using the `quiet=T` argument. The tabulated output can be written to the working directory as a text file by setting the argument `writetable=T`.

__4.3 summariseRunsStructure/summariseRunsTess/summariseRunsAdmixture__  

The table produced using `tabulateRunsStructure()`, `tabulateRunsTess()` or `tabulateRunsAdmixture` can be further collapsed by *K* based on number of runs. The output table from `tabulateRunsStructure()`, `tabulateRunsTess()` or `tabulateRunsAdmixture` can be passed as input to `summariseRunsStructure()`, `summariseRunsTess()` and `summariseRunsAdmixture` respectively. 

```{r,echo=TRUE, eval=FALSE}
# basic usage
sr1 <- summariseRunsStructure(data=tr1)
sr2 <- summariseRunsTess(tr2)
sr3 <- summariseRunsTess(tr3)
...
sr3 <- summariseRunsAdmixture(data=tabulateRunsAdmixture(files=choose.files(multi=TRUE)))
...
The `summariseRunsTess()` and `summariseRunsAdmixture()` functions return 3 columns namely individuals, *K* and number of runs. The summarised runs can be written to the working directory as a text file by setting the argument `writetable=T`.

__4.4 evannoMethodStructure__  

The `evannoMethodStructure()` function is only applicable to STRUCTURE runs.

The Evanno method^2^ is used to estimate the number of *K*. The method is based on Evanno *et al*., (2005) listed in references. The summarised runs table output from`summariseRunsStructure()` function can be input to `evannoMethodStructure()`. The `evannoMethodStructure()` function creates an Evanno derivative plot if suitable conditions are met. A resulting table is also returned. The plot can be written to file using argument `exportplot=T`. The table can be written to file using `writetable=T`.
...
__4.5 runsToDfStructure/runsToDfTess/runsToDfAdmixture__  

STRUCTURE, TESS or ADMIXTURE run files can be converted to R dataframes using the functions `runsToDfStructure()`, `runsToDfTess()` or `runsToDfAdmixture()` respectively. If one file is selected, a dataframe is returned. If multiple files are selected, then a list of dataframes are returned.

```{r,echo=TRUE, eval=FALSE}
# basic usage
rtd1 <- runsToDfStructure(files=slist)
rtd2 <- runsToDfTess(files=tlist)
rtd3 <- runsToDfAdmixture(files=alist)

# select files interactively
rtd1 <- runsToDfStructure(files=choose.files(multi=TRUE))
rtd2 <- runsToDfTess(files=choose.files(multi=TRUE))
rtd3 <- runsToDfAdmixture(files=choose.files(multi=TRUE))
```
...
__4.6 clumppExportStructure/clumppExportTess/clumppExportAdmixture__  
...
![clumpp folder preview](Fig3.jpg)  
__Fig. 5.__ *Folders created from clumpp export and the contents of each folder.*

The functions `clumppExportStructure()`, `clumppExportTess()` and `clumppExportAdmixture()` takes multiple STRUCTURE, TESS or ADMIXTURE files and combines runs by *K* and generates a combined data file and a paramfile in separate directories. The name for the folder can be change optionally if required using the argument `prefix="something"`.

```{r,echo=TRUE, eval=FALSE}
# basic usage
clumppExportStructure(files=slist)
clumppExportTess(files=tlist)
clumppExportTess(files=alist)

# select files interactively
clumppExportStructure(files=choose.files(multi=TRUE))
clumppExportTess(files=choose.files(multi=TRUE))
clumppExportAdmixture(files=choose.files(multi=TRUE))
...
![clumpp folder results](Fig4.jpg)  
...
# basic usage
setwd(choose.dir())
# collect aligned files from STRUCTURE CLUMPP results
collectClumppOutput()
# collect aligned files from TESS CLUMPP results
collectClumppOutput(prefix="TESSpop")
# collect aligned files from ADMIXTURE CLUMPP results
...
collectClumppOutput(prefix="ADMIXTUREpop",filetype="both",runsdir=choose.dir())
...
The function `plotRuns()` is used to create barplots from STRUCTURE run files, TESS run files, ADMIXTURE run files, combined files, aligned files or merged files. When plotting STRUCTURE, TESS or ADMIXTURE run files, use `imgoutput="sep"` or `imgoutput="join"`. When `imgoutput="sep"`, each run file is plotted and exported as separate figure. When `imgoutput="join"`, all selected run files are plotted as a single figure and exported. When plotting combined, aligned or merged files, set `imgoutput="tab"`.

![plotRuns Overview](plotRunsOverview.jpg) 
...
plotRuns(files=alist[1:2],imgoutput="sep")
...
apops <- read.delim(system.file("files/admixturepoplabels.txt", package="pophelper"), header=F)
head(pops$V1)
length(pops$V1)
...
![Interpolation sample 1](Fig10.jpg)  
...
![Interpolation sample 2](Fig11.jpg)  
...
![Interpolation sample 3](Fig12.jpg)  
...
dev.off()
```
![plotRunsSpatial sample 1](Fig13.jpg)  
...
#tabulate runs
df1 <- tabulateRunsAdmixture(alist)
#summarise runs
df2 <- summariseRunsAdmixture(df1)
#clumpp export
clumppExportAdmixture(alist)
#collect clumpp output
collectClumppOutput(prefix="ADMIXTUREpop",filetype="aligned")
#plot runs
