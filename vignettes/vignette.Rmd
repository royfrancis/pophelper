...
The function `plotQ()` is used to create barplots from a qlist. All individuals are shown on a single row. The focus is on groups and not individuals. A schematic of the plot parts is shown below.  

![plotq-overview](plotq-overview.png) 
__Fig. 7:__ *An overview of the components of a `plotQ()` figure and some of the arguments used to modify them.*
...
When using `sortind="label"`, individuals are sorted by individual labels. Individual labels although not displayed on the plots, are present in the underlying data. Individual labels depend on the row labels of the input data, `useindlab` and `grplab`. 

The default `useindlab=F`, creates labels numerically in the original order of data but with zero padding. For example, if there are 10 individuals, labels are 01, 02 up to 10. if there are 100 individuals, then labels are 001, 002 up to 100. Zero padding to ensure optimal sorting. When `useindlab=T`, labels are used from rownames of qlist dataframes. They are usually labelled 1,2,3.. if read in using `readQ()`. This can be an issue with sorting by labels `sortind="label"`. For STRUCTURE files with individual labels, they can be read in automatically using `readQ(indlabfromfile=T)`. The rownames of qlist can also be modified by the user. When group labels are in use, `grplab`, they are concatenated to the individual labels in both cases `useindlab=T` and `useindlab=F` separated by `labsep`.  Default `labsep=" "` adds a space between individual label and grplab. For example, group labels 'popA', 'popA'... will be '01 popA', '02 popA'... when `useindlab=F` and usually '1 popA', '2 popA'... when `useindlab=T`. When multiple group labels are in use, the are similarly concatenated one after the other to individual names in the order in which the group labels were provided. The full concatenated string will determine the final sort order of the individuals. The option `sortind="label"` is not particularly useful or intended for use with `plotQ()`, since individual labels are not visible. It is more useful with `plotQMultiline()`. This is only provided here to recreate an order produced in `plotQMultiline()`.
...
# adjust marker line
plotQ(slist[1], grplab=list("lab1\n"=grp1$V1), linesize=0.5, linecol="steelblue")
plotQ(slist[1], grplab=list("lab1\n"=grp1$V1), linecol="steelblue", linetype=3, linesize=0.2)
...
```{r, echo=TRUE, eval=FALSE}
plotQ(slist[1] ,grplab=list("lab1\n"=grp1$V1,"lab2\n"=grp2$V1), linesize=3, linealpha=0.2, pointcol="white", pointsize=2, labpos=0.5, linepos=0.5, labpanelheight=0.75)
```
...
plotQ(slist[1],grplab=list("lab1\n"=grp1$V1),divcol="black",divtype=1,divsize=1)
```
...
plotQ(slist[c(1,3)],grplab=list("lab1\n"=grp1$V1),sortind="label",imgoutput="join")
```
...
__Fig. 22:__ *Plot with custom font.*
...
plotQMultiline(readQ(al),spl=75,lpp=2,indlab=T,yaxislabs=T,ticks=T,height=5,width=20)
# modified spl
plotQMultiline(readQ(me),spl=38,height=8,width=14)
```

![plotqmultiline](plotqmultiline.png)  
__Fig. 24:__ *Left: Default output from `plotQMultiline`. Right: Modified output with custom `spl` and `lpp`.*

Individual labels are displayed on the plot when `indlab=T`. They can be turned off by `indlab=F`. Note that this only controls the display of individual labels on the plot. The labels are still present in the underlying data and are still in use for sorting etc.
...
__Fig. 25:__ *Various sorting options in plotQMultiline(). Original order, ordered by Cluster1, ordered by all clusters and ordered by individual labels*

### Group labels

One or more sets of group labels can be added by providing a character vector of labels. The length of each vector must be equal to the number of individuals. 

When group labels are in use, `grplab`, they are concatenated to the individual labels in both cases `useindlab=T` and `useindlab=F` separated by `labsep`.  Default `labsep=" "` adds a space between individual label and grplab. For example, group labels 'popA', 'popA'... will be '01 popA', '02 popA'... when `useindlab=F` and usually '1 popA', '2 popA'... when `useindlab=T`. When multiple group labels are in use, the are similarly concatenated one after the other to individual names in the order in which the group labels were provided. The full concatenated string will determine the final sort order of the individuals. 

The vectors must be constructed as a named list. In this demonstration, we will use the labels in the `pophelper` library. Make sure that the labels are character datatype. When reading from a text file, use `stringsAsFactors=F` to ensure that each column in the dataframe is a character and not a factor.  
...
Labels are added as named lists. One or more label sets can be added.

```{r,echo=TRUE, eval=FALSE}
# add one set of group labels
plotQMultiline(list1,grplab=list("lab1"=grp1$V1))

# read another grp label set
grp2 <- read.delim(system.file("files/structuregrplabels2.txt", package="pophelper"), header=F,stringsAsFactors=F)
# add second set of group labels
plotQMultiline(list1,grplab=list("lab1"=grp1$V1,"lab2"=grp2$V1))
```

Group labels are concatenated to the individual labels using separator 'labsep'. The default label separator is space. For example, individuals 1 and 2 with group labels popA and popB produces labels '1 popA' and '2 popB'. Any number of provided group labels are concatenated one after the other.
...
![plotqmultiline](plotqmultiline-grplab.png)  
__Fig. 26:__ *plotQMultiline() with group labels. Original order, ordered by Cluster1, ordered by all clusters and ordered by individual labels*


```{r,echo=TRUE, eval=FALSE}
#using ind labs from file
#sort all
plotQMultiline(list1,useindlab=T,grplab=list("lab1"=grp1$V1),sortind="all")
# sort by cluster 1
plotQMultiline(list1,useindlab=T,grplab=list("lab1"=grp1$V1),sortind="Cluster1")
# sort by label
plotQMultiline(list1,useindlab=T,grplab=list("lab1"=grp1$V1),sortind="label")

#ind labs from file not used
#sort all
plotQMultiline(list1,useindlab=F,grplab=list("lab1"=grp1$V1),sortind="all")
# sort by cluster 1
plotQMultiline(list1,useindlab=F,grplab=list("lab1"=grp1$V1),sortind="Cluster1")
# sort by label
plotQMultiline(list1,useindlab=F,grplab=list("lab1"=grp1$V1),sortind="label")
```
...
```{r, echo=TRUE, eval=FALSE}
# subsetting groups
plotQMultiline(list1,grplab=list("lab1"=grp1$V1),subsetgrp="Pop A")
# change order of groups
plotQMultiline(list1,grplab=list("lab1"=grp1$V1),subsetgrp=c("Pop B","Pop A"))

# subsetting group and sorting individuals
plotQMultiline(list1,grplab=list("lab1"=grp1$V1),subsetgrp="Pop A",sortind="Cluster1")
plotQMultiline(list1,grplab=list("lab1"=grp1$V1),subsetgrp="Pop A",sortind="all")

# change order of groups and sorting individuals
plotQMultiline(list1,grplab=list("lab1"=grp1$V1),subsetgrp=c("Pop B","Pop A"),sortind="Cluster1")
```
...
# change bar width
plotQMultiline(list1,useindlab=T,barsize=1,spl=149,lpp=1,labsize=3,height=5)
```

![plotqmultiline-colours](plotqmultiline-colours.png)  
__Fig. 27:__ *Multiline plots with (left) standard colours, (middle) `rich.colors()` from `gplots` package and (right) `brewer.pal(8,"Spectral")` from `RColorBrewer` package.*
...
__Fig. 28:__ *Left: DISTRUCT plot with `grpmean=F`. Middle: DISTRUCT plot with `grpmean=T`. Right: DISTRUCT plot with `grayscale=T`.*  
...
[1] extrafont_0.17  pophelper_2.1.0 ggplot2_2.2.1   Cairo_1.5-9     testthat_1.0.2  devtools_1.12.0

loaded via a namespace (and not attached):
 [1] Rcpp_0.12.10     Rttf2pt1_1.3.4   knitr_1.15.1     magrittr_1.5     munsell_0.4.3   
 [6] colorspace_1.3-2 R6_2.2.0         stringr_1.2.0    dplyr_0.5.0      httr_1.2.1      
[11] plyr_1.8.4       tools_3.3.3      grid_3.3.3       gtable_0.2.0     DBI_0.6-1       
...
[26] tidyr_0.6.1      curl_2.5         memoise_1.0.0    labeling_0.3     stringi_1.1.5   
[31] scales_0.4.1 
```
...
other attached packages:
[1] extrafont_0.17  testthat_1.0.2  pophelper_2.1.0 ggplot2_2.2.1  
[5] Cairo_1.5-9    

loaded via a namespace (and not attached):
 [1] Rcpp_0.12.10     tidyr_0.6.1      crayon_1.3.2     grid_3.3.3      
 [5] plyr_1.8.4       R6_2.2.0         Rttf2pt1_1.3.4   gtable_0.2.0    
 [9] magrittr_1.5     scales_0.4.1     lazyeval_0.2.0   extrafontdb_1.0 
[13] tools_3.3.3      munsell_0.4.3    colorspace_1.3-2 gridExtra_2.2.1 
[17] tibble_1.3.0 
```
...
other attached packages:
[1] pophelper_2.1.0 ggplot2_2.2.1   Cairo_1.5-9

loaded via a namespace (and not attached):
 [1] colorspace_1.3-2 scales_0.4.1     magrittr_1.5     lazyeval_0.2.0
 [5] plyr_1.8.4       gtable_0.2.0     tibble_1.3.0     gridExtra_2.2.1
 [9] Rcpp_0.12.10     grid_3.3.2       tidyr_0.6.1      munsell_0.4.3
