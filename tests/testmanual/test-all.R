...
for(i in seq_along(ftype))
{
  ...
  cr <- pophelper:::checkQ(files)
    ...
    if(unique(cr$type) == "BASIC")
    {
      ...
      expect_error(pophelper::readQ(files,filetype="structure"))
      #expect_warning(pophelper::readQ(files,filetype="tess"))
      expect_error(pophelper::readQ(files,filetype="baps"))
      #expect_warning(pophelper::readQ(files,filetype="clumpp"))
    }
    ...
    tr <- pophelper::tabulateQ(qlist=xlist,writetable=TRUE)
