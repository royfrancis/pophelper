...
getColors <- getColours <- function(k)
{
  ...
  if(k > 12)
  {
    cr <- grDevices::colorRampPalette(colors=c("#000040FF","#00004FFF","#000060FF","#000074FF","#000088FF","#00009DFF","#0000B2FF",
...
tabulateQ <- function(qlist=NULL,writetable=FALSE,sorttable=TRUE,exportpath=NULL)
{
  ...
  if(writetable) utils::write.table(main,paste0(exportpath,"tabulateQ.txt"),quote=FALSE,row.names=FALSE,sep="\t",dec=".")
...
summariseQ <- summarizeQ <- function(data=NULL,writetable=FALSE,exportpath=NULL)
{
  ...
  # write table if opted
  if(writetable) utils::write.table(dframe1,paste0(exportpath,"summariseQ.txt"),quote=FALSE,row.names=FALSE,sep="\t",dec=".")
                                  ...
                                  theme="theme_bw",font="",na.rm=TRUE,quiet=TRUE,outputfilename="evannoMethodStructure",exportpath=NULL)
{
  ...
  if(err==1)
  {
    if(exportplot || returnplot)
    {
      ...
      {
        # check image imgtype
        if(imgtype=="pdf") grDevices::pdf(paste0(exportpath,outputfilename,".pdf"),height=height1,width=width1,fonts=font)
        if(imgtype=="png") grDevices::png(paste0(exportpath,outputfilename,".png"),height=height1,width=width1,res=dpi,units=units,family=font)
        if(imgtype=="jpeg") grDevices::jpeg(paste0(exportpath,outputfilename,".jpg"),height=height1,width=width1,res=dpi,units=units,quality=100,family=font)
        if(imgtype=="tiff") grDevices::tiff(paste0(exportpath,outputfilename,".tiff"),height=height1,width=width1,res=dpi,units=units,compression="lzw",family=font)
  ...
  if(writetable)
  {
    utils::write.table(data,paste0(exportpath,outputfilename,".txt"),quote=FALSE,row.names=FALSE,sep="\t",dec=".")
  ...
  for(i in seq_along(qlist))
  {
    ...
    for(j in 1:tab_runs)
    {
      ...
      if((!any(is.na(grplabbottom))) && (!any(is.na(grplabtop))))
      {
        ...
        dfb2 <- cbind(stats::aggregate(value~grp+variable,data=dfb1,FUN=mean),stats::aggregate(value~grp+variable,data=dfb1,FUN=length)[,3,drop=FALSE])
        ...
        dft2 <- cbind(stats::aggregate(value~grp+variable,data=dft1,FUN=mean),stats::aggregate(value~grp+variable,data=dft1,FUN=length)[,3,drop=FALSE])
      ...
      if((!any(is.na(grplabbottom))) && (any(is.na(grplabtop))))
      {
        ...
        df2 <- cbind(stats::aggregate(value~grp+variable,data=df1,FUN=mean),stats::aggregate(value~grp+variable,data=df1,FUN=length)[,3,drop=FALSE])
      ...
      if((any(is.na(grplabbottom))) && (any(is.na(grplabtop))))
      {
        ...
        df2 <- cbind(stats::aggregate(value~variable,data=df1,FUN=mean),stats::aggregate(value~variable,data=df1,FUN=length)[,2,drop=FALSE])
      ...
      utils::write.table(grpq,fname_grpq,row.names=FALSE,col.names=FALSE,quote=FALSE,dec=".",sep="\t")
      if(!quiet) cat(paste0(fname_grpq," exported.\n"))
      utils::write.table(indq,fname_indq,row.names=FALSE,col.names=FALSE,quote=FALSE,dec=".",sep="\t")
      if(!quiet) cat(paste0(fname_indq," exported.\n"))
      if(!any(is.na(grplabbottom)))
      {
        utils::write.table(plb,fname_plb,row.names=FALSE,col.names=FALSE,quote=FALSE,dec=".",sep="\t")
        if(!quiet) cat(paste0(fname_plb," exported.\n"))
      }
      if(!any(is.na(grplabtop)))
      {
        utils::write.table(plt,fname_plt,row.names=FALSE,col.names=FALSE,quote=FALSE,dec=".",sep="\t")
        if(!quiet) cat(paste0(fname_plt," exported.\n"))
      }
      utils::write.table(colsdf,fname_col,row.names=FALSE,col.names=FALSE,quote=FALSE,dec=".",sep="\t")
