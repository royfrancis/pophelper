...
#' @param useindlab A logical indicating if individual labels must be read from the rownames of qlist dataframes and used as individual labels.
...
plotQ <- function(qlist=NULL,imgoutput="sep",clustercol=NA,sortind=NA,grplab=NA,selgrp=NA,ordergrp=FALSE,subsetgrp=NA,grpmean=FALSE,panelspacer=0.06,
                    showsp=TRUE,splab=NA,splabsize=4,splabcol="grey30",splabface="plain",spbgcol=NA,sppos="right",
                    grplabspacer=0,grplabheight=NA,grplabpos=0.25,grplabsize=NA,grplabangle=NA,grplabjust=NA,grplabcol="grey30",grplabalpha=1,
                    useindlab=FALSE,indlabsep=" ",showindlab=FALSE,indlabheight=0,indlabsize=3,indlabangle=90,indlabvjust=0.5,indlabhjust=1,indlabcol="grey30",
                    ...
                    showlegend=FALSE,legendlab=NA,legendpos="left",legendkeysize=4,legendtextsize=3,
                    ...
                    barsize=1,theme="theme_grey",font="",na.rm=TRUE)
{
  ...
  if(imgoutput=="sep")
  {
    for (i in 1:flen)
    {
      ...
      if(any(is.na(outputfilename))) outname <- fname
      if(!all(is.na(outputfilename))) outname <- outputfilename[i]
      ...
      if(!grplabcheck)
      {
        ...
        #add showindlab
        if(!showindlab) p <- p+theme(axis.text=element_blank())
      ...
      if(grplabcheck)
      {
        ...
        #add showindlab
        if(!showindlab) p1 <- p1+theme(axis.text=element_blank())
  ...
  if(imgoutput=="join")
  {
    ...
    #output name
    if(any(is.na(outputfilename))) outname <- paste0("Joined",flen,"Files-",as.character(format(Sys.time(),"%Y%m%d%H%M%S")))
    if(!all(is.na(outputfilename))) outname <- outputfilename
...
#' @param showticks A logical indicating if ticks on axis should be displayed or not.
#' @param showyaxis A logical indicating if y-axis labels should be displayed or not.
#' @param outputfilename A character or character vector denoting output file name without file extension. See details. 
...
plotQMultiline <- function(qlist=NULL,spl=NA,lpp=NA,clustercol=NA,sortind=NA,grplab=NA,selgrp=NA,ordergrp=FALSE,subsetgrp=NA,grpmean=FALSE,
                            ...
                            indlabsize=5,indlabangle=90,indlabvjust=0.5,indlabhjust=1,indlabcol="grey30",
                            showgrplab=TRUE,grplabsize=7,grplabcol="grey30",grplabbgcol="#DCDCDC",
                            barsize=0.9,showticks=FALSE,showyaxis=FALSE,
                            outputfilename=NA,imgtype="png",height=NA,width=NA,dpi=300,units="cm",mar=c(0.1,0.1,0.1,0),
      ...
      for (j in 1: nr2)
      {
        ...
        plist[[j]] <- ggplot2::ggplot(data=df2,aes(x=ind,y=value,fill=variable))+
          ...
          theme(legend.position="none",
                ...
                axis.line=element_blank(),
                axis.title=element_blank(),
                axis.text.x=element_text(size=indlabsize,angle=indlabangle,vjust=indlabvjust,hjust=indlabhjust,colour=indlabcol),
...
#ON LOAD
.onLoad <- function(...) {
  packageStartupMessage("pophelper v2.2.0 ready.")
}
